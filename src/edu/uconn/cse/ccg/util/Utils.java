 ...
 *
 * Cfinder is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Lesser General Public License as published by
 * the Free Software Foundation, either version 3 of the License, or
 * (at your option) any later version.
 *
 * Cfinder is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU Lesser General Public License for more details.
 *
 * You should have received a copy of the GNU Lesser General Public License
 * along with Cfinder.  If not, see <http://www.gnu.org/licenses/>.
 */
...
//		System.out.println(info);
	...
	public static String readInputStreamToString(InputStream stream,
		...
		Reader r = new BufferedReader(new InputStreamReader(stream, encoding),
				16384);
	...
	public static void parseExpression(Map<String, String> usedClass, int type, CompilationUnit cu, Expression expr) {
		...
		if (expr instanceof ArrayAccess) {
			...
			//System.out.println("VariableDeclarationExpression " + variableDeclarationExpression.toString());		
			...
			parseExpression(usedClass, LineInfo.RIGHT_HANDSIDE, cu, ((Assignment) expr).getRightHandSide());
			//			parseExpression(usedClass, LineInfo.LEFT_HANDSIDE, cu, ((Assignment) expr).getLeftHandSide());
			//			parseExpression(usedClass, LineInfo.RIGHT_HANDSIDE, cu, ((Assignment) expr).getRightHandSide());
			...
			parseExpression(usedClass, LineInfo.CLASSINSTANCECREATION, cu, classInstanceCreation.getExpression());
			...
			parseExpression(usedClass, LineInfo.THEN_EXPRESSION, cu, conditionalExpression.getThenExpression());
			parseExpression(usedClass, LineInfo.ELSE_EXPRESSION, cu, conditionalExpression.getElseExpression());
			...
			parseExpression(usedClass, LineInfo.LEFT_INFIXEXPRESSION, cu, infixExpression.getLeftOperand());
			parseExpression(usedClass, LineInfo.RIGHT_INFIXEXPRESSION, cu, infixExpression.getRightOperand());
			...
			parseExpression(usedClass, LineInfo.PARENTHESIZEDEXPRESSION, cu, parenthesizedExpression.getExpression());
			...
			//			//System.out.println("MethodInvocation " + methodInvocation.resolveMethodBinding().getName() + " : " + methodInvocation.resolveMethodBinding().getDeclaringClass().getQualifiedName());

			if (methodInvocation != null && methodInvocation.resolveMethodBinding() != null && methodInvocation.resolveMethodBinding().getDeclaringClass()!=null) {
				usedClass.put(methodInvocation.resolveMethodBinding().getName() + ":" + cu.getLineNumber(methodInvocation.getStartPosition())
						+ ":" + cu.getColumnNumber(methodInvocation.getStartPosition()) + ":" + cu.getLineNumber(methodInvocation.getStartPosition() + methodInvocation.getLength())
						+ ":" + cu.getColumnNumber(methodInvocation.getStartPosition() + methodInvocation.getLength())
						+ ":" + methodInvocation.getStartPosition(), methodInvocation.resolveMethodBinding().getDeclaringClass().getQualifiedName());
	...
	public static void parseDetailExpression(int type, CompilationUnit cu, Expression expr) {
		...
		if (expr instanceof ArrayAccess) {
			...
			////				System.out.println("Qualified Name " + name.getFullyQualifiedName() + " of " + ((QualifiedName)name).getName().getFullyQualifiedName());
			//			else 
			...
			//System.out.println("ClassInstanceCreation ");
			ClassInstanceCreation classInstanceCreation = (ClassInstanceCreation)expr;
			parseDetailExpression(LineInfo.CLASSINSTANCECREATION, cu, classInstanceCreation.getExpression());
			List<Expression> exprs = classInstanceCreation.arguments();
			for (Expression exp : exprs) {
				//System.out.println("Expr " + exp);
			...
			//System.out.println("ParenthesizedExpression ");
			ParenthesizedExpression parenthesizedExpression = (ParenthesizedExpression)expr;
			parseDetailExpression(LineInfo.PARENTHESIZEDEXPRESSION, cu, parenthesizedExpression.getExpression());
		} else if (expr instanceof PostfixExpression) {
			//System.out.println("PostfixExpression ");
		} else if (expr instanceof PrefixExpression) {
			//System.out.println("PrefixExpression ");
			...
			if (methodInvocation.resolveMethodBinding() != null) {
				//			usedClass.put(methodInvocation.resolveMethodBinding().getName() + ":" + cu.getLineNumber(methodInvocation.getStartPosition()) + ":" + methodInvocation.getStartPosition(), methodInvocation.resolveMethodBinding().getDeclaringClass().getQualifiedName());
...
//						System.out.println("right InfixExpression" + infixExpression.getRightOperand());
	...
	public static String parseBinding(ASTNode node, IBinding fBinding) {
		...
		case IBinding.VARIABLE:
			...
			return (variableBinding.getDeclaringClass() != null) ? variableBinding
					.getDeclaringClass().getQualifiedName() : "";
					// variableBinding.
					// break;
	...
	public static void parseMethodInvocation(Map<String, String> usedClass, int type, CompilationUnit cu, MethodInvocation node) {
	...
	public static LineInfo getClosestAssignmentStatement(int lineNumber, Map<Integer, LineInfo> lineToNode) {
	...
	//unchecked
	public static List<LineInfo> getInfoFromMethodInvocation(String methodName, int nbParameters, Map<Integer, LineInfo> lineToNode) {
		//		LineInfo result = new LineInfo(-1, -1, -1, "");
		...
		for (i = 0; i <keys.length; i++) {
			...
			if (node.type == LineInfo.METHODINVOCATION)
				if (methodName.equals(node.name)) {
					//					System.out.println("Found method " + node.name + " has " + node.parameters.size() + " need to have " + nbParameters);
					//					if (node.parameters.size() == nbParameters)  //need to check later to get the correct method of a class. A class might have many method with the same name
					result.add(node);
					//						return node;			
				}
		}
	...
	//unchecked
	public static LineInfo getInfoFromMethodDeclaration(String methodName, int nbParameters, Map<Integer, LineInfo> lineToNode) {
		//		LineInfo result = new LineInfo(-1, -1, -1, "");
		LineInfo result = null;
		int[] keys = new int[lineToNode.keySet().size()];
		int i = 0;
	...
	public static String getASTNodeType(ASTNode node) {
		...
		//all node types below this line are used by other analyses which are called within analyzeParentStatement
